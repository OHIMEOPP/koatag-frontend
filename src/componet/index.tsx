...
function responseMessage() {
  ...
  return { message, setState, loading };
}
...
function responseImgDate() {
  ...
  const handleClick = async () => {
    if (inputRef.current) {
      fetch("http://localhost:6001/submit", {
        ...
        body: JSON.stringify({ colum: searchValue, fieldName: searchField, searchName: searchName }) // 将数据对象转换为 JSON 字符串
      })
        .then((response) => response.json())
        .then(data => {
          setVv(data);
  ...
  return { inputRef, handleClick, searchValue, setSerchValue, vv, searchField, setSerchField, searchName, setSerchName }
}

function App() {

  const { message, loading, setState } = responseMessage();
  ...
  const { inputRef, handleClick, searchValue, setSerchValue, vv, searchField, setSerchField, searchName, setSerchName } = responseImgDate();
